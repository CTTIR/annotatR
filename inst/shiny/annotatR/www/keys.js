...
(function () {
  ...
  document.addEventListener("keydown", function (ev) {
    ...
    else if (k === " ") Shiny.setInputValue("key_overlay", Date.now(), { priority: "event" });
    ...
    else if (k === "Enter" && ev.shiftKey) Shiny.setInputValue("key_commit_advance", Date.now(), { priority: "event" });
    else if (k === "Enter") Shiny.setInputValue("key_commit", Date.now(), { priority: "event" });
    ...
    } else if ((ev.ctrlKey || ev.metaKey) && k.toLowerCase() === "e") {
      ev.preventDefault();
